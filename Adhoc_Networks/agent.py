...
class Agent():
    def __init__(self, adhocnet, flow_id, reward_type):
        assert reward_type in ["Q-Min", "Q-MC", "Q-Sum"]
        ...
        self._model_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                        "DQN/Trained_Models/", "agent_{}_neighbors_{}_rewards.pt".format(self.n_nodes_explored, self.reward_type))
    ...
    def save_dqn_model(self):
        ...
        print("Agent {}: Saved DQN_{} model.".format(self.id, self.reward_type))
        return
    ...
    def process_links(self, memory):
        ...
        for tx, band, rx, state, action in links[::-1]:
            if tx != rx: # not a reprobe transaction
                ...
                if self.reward_type in ["Q-Min", "Q-Sum"]:
                    reward = self.normalize_reward(SINR)
                else: 
                    # directly get the future bottleneck reward for Monte-Carlo estimation
                    reward = self.normalize_reward(np.min(SINRs_onwards))  
            else: 
                if self.reward_type == "Q-Min":
                    # for Q-Min reward scheme: just have a very large reward assigned so it doesn't affect Q val
                    reward = np.inf
                elif self.reward_type == "Q-Sum":
                    reward = 0
                else:
                    reward = self.normalize_reward(np.min(SINRs_onwards)) 
        ...
        if memory!=None: # have to compare to None, since replay memory object has __len__() defined, empty would result in false
            for transaction in transactions[::-1]: # ordering matters within the replay buffer, to get next state in the memory efficient fashion
                memory.add(transaction)
        return

    def normalize_reward(self, val):
        # return np.log10(val / 1e2) # normalize rate
    ...
    def train(self, memory):
        ...
        next_Qs = self.target_net_predict(next_states)[np.arange(minibatch_size), next_actions].detach().cpu().numpy()
        # Compute Loss
        if self.reward_type == "Q-Min":
            targets_tmp = np.minimum(rewards, next_Qs * (1 - whether_done.astype(float)))
            rewards[rewards==np.inf] = 100   # to avoid np.inf*0 for reprobe transactions (guaranteed with not done)
            targets = targets_tmp + rewards * whether_done.astype(float)
            loss = self.loss_func(Qs, torch.tensor(targets, dtype=torch.float32).to(DEVICE))
        elif self.reward_type == "Q-MC":
            loss = self.loss_func(Qs, torch.tensor(rewards, dtype=torch.float32).to(DEVICE))
        else:
            targets = rewards + next_Qs * (1 - whether_done.astype(float))
            loss = self.loss_func(Qs, torch.tensor(targets, dtype=torch.float32).to(DEVICE))
        loss.backward()
        self.optimizer.step()
        ...
        for tx, band, rx, _, _ in links:
            if tx != rx:  # a non-reprobe action
                ax.arrow(x=self.adhocnet.nodes_locs[tx][0], y=self.adhocnet.nodes_locs[tx][1],
                         ...
                         length_includes_head=True, head_width=6, linestyle=BAND_LINESTYLES[band], color=self.flow.plot_color, linewidth=1.85)
        return
    ...
    def visualize_non_zero_rewards(self, memory):
        ...
        plt.title("Non-zero reward distribution in replay memory with reward type {}".format(self.reward_type))
